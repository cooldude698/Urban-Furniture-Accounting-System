...
export class VendorBillService {
  static async getAll(): Promise<VendorBillDTO[]> {
    ...
    return bills.map(b => ({
      ...
      billDate: b.bill_date instanceof Date ? b.bill_date.toISOString().split('T')[0] : String(b.bill_date),
      dueDate: b.due_date instanceof Date ? b.due_date.toISOString().split('T')[0] : (b.due_date ? String(b.due_date) : null),
      status: b.status,
