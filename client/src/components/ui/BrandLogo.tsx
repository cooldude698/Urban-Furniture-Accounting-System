...
export interface BrandLogoProps {
  size?: number;
  variant?: 'dark' | 'light' | 'cream' | 'accent';
  showText?: boolean;
  subtitle?: string;
  badge?: boolean;
  className?: string;
...
/**
 * ChairIcon: Vector Monomark of Urban Furniture
 * Renders the chair silhouette symbol cleanly at any scale
 */
export const ChairIcon: React.FC<{
  ...
  return (
    <svg
      ...
      aria-label="Urban Furniture Monomark"
    >
      {/* Upper flowing chair body (backrest, seat curve, front leg) */}
...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  const iconColor = isLight ? 'var(--cream, #FBF9F5)' : isCream ? 'var(--cream, #FBF9F5)' : 'var(--brown-900, #261914)';
  const textColor = isLight ? 'var(--cream, #FBF9F5)' : 'var(--brown-900, #261914)';
  const subColor = isLight ? 'var(--brown-300, #C2A792)' : 'var(--brown-600, #77574A)';
  const badgeBg = isLight ? 'var(--brown-700, #4A3528)' : 'var(--cream, #FBF9F5)';
  const badgeBorder = isLight ? 'rgba(255, 255, 255, 0.12)' : 'var(--brown-200, #DFD3C3)';

  const iconElement = (
    <ChairIcon size={badge ? Math.round(size * 0.65) : size} color={iconColor} />
  );
  ...
  return (
    <div
      ...
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: Math.max(8, Math.round(size * 0.3)),
        textDecoration: 'none',
        userSelect: 'none',
        ...style,
      }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.1 }}>
          <span
            style={{
              ...
              fontWeight: 700,
              fontSize: Math.max(14, Math.round(size * 0.56)),
